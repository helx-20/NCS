...
class SNN_Model_4LIF(nn.Cell):
    ...
    def __init__(self, n_tasks):
        ...
        self.fc1 = nn.Dense(36*36*1, cfg_fc[0])
        self.fc = nn.Dense(cfg_fc[0], cfg_fc[1])

    def construct(self, input, win=15):
        batch_size = input.shape[0]
        h1_mem = h1_spike = Tensor(np.zeros((batch_size, cfg_fc[0])), mindspore.float32)
        h1_sumspike = Tensor(np.zeros((batch_size, cfg_fc[0])), mindspore.float32)
